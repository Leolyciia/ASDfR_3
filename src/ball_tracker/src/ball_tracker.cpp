...
//   // Parameters
//   h_min_ = 21; h_max_ = 102;
//   s_min_ = 82; s_max_ = 255;
//   v_min_ = 104; v_max_ = 255;
...
BallTracker::BallTracker() : Node("ball_tracker") {
  ...
  params.minArea = CV_PI * (size_min_px_ * size_min_px_) / 4.0;
  params.maxArea = CV_PI * (size_max_px_ * size_max_px_) / 4.0;
...
void BallTracker::image_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
  ...
  if (best_size > 0) {
    ...
    char text_buffer[50];
    snprintf(text_buffer, sizeof(text_buffer), "Size: %.1f px", best_size);
    cv::putText(display_image, text_buffer, cv::Point(10, 25), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 0), 2);
    geometry_msgs::msg::Point p;
    p.x = best_center.x;
    p.y = best_center.y;
    p.z = best_size;
    ...
    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "No valid ball detected.");
    geometry_msgs::msg::Point p_no_ball;
    p_no_ball.x = -1.0;
    p_no_ball.y = -1.0;
    p_no_ball.z = 0.0;
